# V2 — Panel Recommendation Insights Scorecard
...
## Panel Assessment Layer

### Overview

After the screening manager completes the initial SCALE scorecard (application-based), the venture proceeds to a **panel discussion**. The panel assessment adds two components on top of the screening scorecard:

1. **Panel Recommendation Rating** — An additional Red/Yellow/Green column on the same 7-dimension scorecard, derived from the panel discussion transcript. This rating sits alongside the application-based rating so both are visible side by side. Panelists can edit the AI-generated panel rating.
2. **Panel Gate Questions** — 6 Yes/No gate questions across 3 categories (Alternatives, Mindset, Resources) that the panelist answers after the discussion. Each question has an optional Remarks field.

---

### Panel Scorecard — Dual-Column Rating

After the panel call, the screening scorecard is extended with two additional columns:

| Column | Source | Editable? |
|--------|--------|-----------|
| **Application Rating** | AI-generated from application data (screening stage) | No — locked after screening |
| **Application Brief** | AI-generated 2-sentence explanation from screening stage | No — locked after screening |
| **Panel Recommendation** | AI-generated from panel discussion transcript, then editable by panelist | Yes — panelist can override |
| **Panel Brief** | AI-generated 2-sentence explanation based on panel discussion | No — AI-generated, read-only |
| **Panel Remarks** | Free-text field for panelist comments per dimension | Yes — optional |

The panelist sees the full scorecard with all columns side by side, and can override the AI-suggested panel rating for any dimension.

#### Frontend Rendering — Panel View

| SCALE Dimension | Assessment | Application Rating | Application Brief | Panel Recommendation | Panel Brief | Panel Remarks |
|-----------------|-----------|-------------------|-------------------|---------------------|-------------|---------------|
| Size | Current Revenue | 🟢 Green | The venture reports ₹45Cr ARR, within the Core range (₹25Cr–₹85Cr). No size concerns for program eligibility. | 🟢 Green | Panel confirmed current ARR of ₹45Cr and discussed pipeline visibility for next 2 quarters. No change to assessment — revenue scale is well within Core thresholds. | — |
| Sector | Growth Sector | 🟢 Green | The venture operates in the industrial automation sector, growing at ~12% CAGR. Well above GDP growth, indicating a strong tailwind. | 🟡 Yellow | Panel discussion revealed the venture's specific sub-segment (legacy PLC retrofitting) faces pricing pressure from Chinese imports. While the broader sector is strong, the niche sub-segment outlook is more cautious. | Needs deeper market validation |
| Capital | Balance Sheet Strength | 🟡 Yellow | The venture reports positive cash flow but has not disclosed PAT status. Clarification needed on profitability. | 🟢 Green | Founder clarified during the call that the company has been PAT-positive for the last 3 years with 8% net margins. This upgrades the capital assessment to Green — strong balance sheet with no debt concerns. | — |
| Ambition | Revenue Addition | 🟢 Green | The 3Y target of ₹90Cr from a ₹45Cr base implies a ~26% CAGR, well above 8% threshold. On track to double revenue within the program period. | 🟢 Green | Founder walked through a bottoms-up revenue build-up during the panel that substantiates the ₹90Cr target. The growth plan is backed by specific customer pipeline and product roadmap milestones. | — |
| Leadership | Committed Team | 🟡 Yellow | The founder has committed to personal involvement in the program. However, no second-in-line management team has been identified. | 🟢 Green | Founder introduced the COO during the panel call, who demonstrated strong operational knowledge and will lead day-to-day execution. Both time commitment and second-line team are now confirmed. | COO seems very capable |
| Jobs / Employment Generation Potential | Direct Jobs Creation (3Y) | 🟢 Green | The venture plans to hire 80+ employees over 3 years, exceeding the Core threshold of 50 jobs. Strong job creation signal. | 🟢 Green | Not discussed in panel — application rating carried forward. The planned hiring of 80+ roles remains the basis for this assessment. | — |
| Venture Clarity | New Venture Definition | 🟡 Yellow | The new product (IoT sensors for predictive maintenance) is well-defined. However, the target segment is described generically as "manufacturing companies." | 🟢 Green | Panel probed the segment definition and the founder specified three target verticals: automotive OEMs, pharmaceutical plants, and cement manufacturers. This resolves the clarity gap — all selected growth dimensions are now well-defined. | — |

---

### Panel Scorecard — AI Prompt for Panel Recommendation
...
| **Allowed Roles** | `panel_member`, `success_mgr`, `venture_mgr`, `committee_member`, `admin` |

#### When Used

Triggered after the panel discussion call is completed and the transcript is available. The panelist or screening manager clicks **"Generate Panel Assessment"** on the Panel Dashboard.

#### Input Data

```typescript
interface PanelAssessmentInput {
    venture_id: string;
    screening_scorecard: ScorecardDimension[];   // The 7-dimension application-based scorecard from screening stage
    panel_transcript: string;                     // Full transcript of the panel discussion call (truncated at 15,000 characters)
...
```
You are an expert panel analyst for the Wadhwani Accelerate Assisted Growth Platform. You have been given:
1. The application-based SCALE scorecard (7 dimensions, each rated Green/Yellow/Red with a brief).
2. The transcript of the panel discussion with the venture's founder/team.

Your task is to produce a **Panel Recommendation** rating for each of the 7 scorecard dimensions, based on what was discussed in the panel call. The panel rating may agree with or differ from the application rating — it reflects new information, clarifications, or concerns that emerged during the discussion.
...
If the panel discussion did not cover a particular dimension, carry forward the application rating and state "Not discussed in panel — application rating carried forward." in the brief.

**Application-Based Scorecard:**
${JSON.stringify(screening_scorecard, null, 2)}

**Panel Discussion Transcript:**
${panel_transcript.slice(0, 15000)}
[... truncated ...]
...
- If the panel discussion revealed new information that changes the rating (up or down), explain what changed. Reference specific statements or clarifications from the transcript.
- If the panel discussion confirmed the application-based assessment, state what was confirmed.
...
interface PanelScorecardDimension {
    dimension: string;
    application_rating: string;    // Carried from screening scorecard
    panel_rating: string;          // "Green" | "Yellow" | "Red" — AI-suggested, editable by panelist
    panel_brief: string;           // 2 sentences based on panel transcript
    panel_remarks?: string;        // Optional — panelist can add free-text remarks per dimension (not AI-generated)
}

interface PanelScorecardInsights {
    panel_scorecard: PanelScorecardDimension[];  // Exactly 7 items
...
After reviewing the scorecard, the panelist answers **6 gate questions** across 3 categories. These are qualitative Yes/No assessments that capture the panelist's judgment on the venture's fit for the program. Each question has an optional Remarks field for additional context.

#### Gate Categories & Questions

**Category 1: Alternatives**

*Can the venture get the support it needs from other sources, or is the Accelerate program uniquely positioned to help?*

| # | Question | Response | Remarks |
|---|----------|----------|---------|
| 1 | Can banks and investors provide the required support based on the venture's balance sheet and business strength/plans? | Yes / No | Optional |
| 2 | Are there other accelerators, consultants, or industry bodies that could provide equivalent growth support to this venture? | Yes / No | Optional |

**Category 2: Mindset**

*Is the owner open to external support, willing to invest in growth, and coachable?*

| # | Question | Response | Remarks |
|---|----------|----------|---------|
| 3 | Is the owner looking for and willing to take operational and strategic support from the program? | Yes / No | Optional |
| 4 | Is the owner willing and open to fund growth sprints and invest in executing the growth plan? | Yes / No | Optional |

**Category 3: Resources**

*Does the venture's growth journey align with what the Accelerate program can deliver?*

| # | Question | Response | Remarks |
|---|----------|----------|---------|
| 5 | Does the owner's growth journey plan fit Wadhwani Foundation's Sprint offering? Can the program provide resources that would benefit this venture? | Yes / No | Optional |
| 6 | Does the venture have the internal capacity (team, bandwidth, infrastructure) to absorb and act on the support provided by the program? | Yes / No | Optional |

#### Gate Questions — Data Schema

```typescript
interface PanelGateQuestion {
    id: number;                    // 1–6
    category: string;              // "Alternatives" | "Mindset" | "Resources"
    question: string;
    response: string;              // "Yes" | "No"
    remarks?: string;              // Optional free-text
}

interface PanelGateAssessment {
    gate_questions: PanelGateQuestion[];  // Exactly 6 items
    submitted_by: string;                  // Panelist name/ID
    submitted_at: string;                  // ISO timestamp
}
```

#### Gate Questions — Interpretation Guidance

The gate questions are **not scored by AI** — they are answered by the panelist based on the discussion. However, the following interpretation logic can be used for dashboard summaries:

| Category | Ideal Response | Concern Signal |
|----------|---------------|----------------|
| **Alternatives** (Q1, Q2) | **No** to both — the venture cannot easily get this support elsewhere, making the program uniquely valuable | **Yes** to both — the venture may not need the program; banks/investors/other accelerators can provide equivalent support |
| **Mindset** (Q3, Q4) | **Yes** to both — the owner is coachable, open to support, and willing to invest in growth | **No** to either — the owner may resist external guidance or may not fund growth sprints |
| **Resources** (Q5, Q6) | **Yes** to both — the program's Sprint offering fits, and the venture can absorb the support | **No** to either — mismatch between the venture's needs and the program's offering, or the venture lacks capacity to act on the support |

#### API Endpoint

```
POST /api/ventures/:id/panel-assessment
```
...
### Panel Assessment — Data Flow

```
Panel Dashboard (frontend)
    │
    ├── Panel call completed → transcript available
    │
    ├── Panelist clicks "Generate Panel Assessment"
    │   ▼
    │   POST /api/ventures/:id/generate-insights?type=panel
    │   ├── 1. Fetch screening scorecard from `ventures.ai_analysis`
    │   ├── 2. Fetch panel transcript
    │   ├── 3. Build panel prompt with screening scorecard + transcript
    │   ├── 4. Call Claude API
    │   ├── 5. Parse JSON response
    │   ├── 6. Return AI-suggested panel ratings to frontend
    │   ▼
    │   Panelist reviews dual-column scorecard
    │   ├── Can edit any panel_rating (Green/Yellow/Red)
    │   ├── Can add panel_remarks per dimension (optional)
    │
    ├── Panelist answers 6 Gate Questions (Yes/No + optional Remarks)
    │
    ├── Panelist clicks "Submit Panel Assessment"
    │   ▼
    │   POST /api/ventures/:id/panel-assessment
    │   ├── Save panel_scorecard + gate_questions to DB
    │   ├── Update `ventures.panel_reviewed_at`
    │   ▼
...
- **v4.2** (2026-03-13): Added Panel Assessment layer — dual-column scorecard (Application Rating + Panel Recommendation from transcript, editable by panelist), optional per-dimension Panel Remarks, 6 Panel Gate Questions (Alternatives, Mindset, Resources) with Yes/No + optional Remarks. Added panel AI prompt, data schemas, API endpoints, and data flow.
